...
value_t ntv_print_value(const value_t* values, uint8_t n, const sv_allocator_t* a)
{
    (void) a;
    assert(n == 1);

    value_t v = values[0];
    sv_str_t str = !IS_STR(v) ? value_to_str(v, &sv_gpa) : AS_STR(v);
...
value_t ntv_print_value_arr(const value_t* values, uint8_t n, const sv_allocator_t* a)
{
    assert(n == 1);

    value_t maybe_list = values[0];
    if (!IS_LIST(maybe_list)) {
        vm_wrong_type_err* paylod = sv_malloc(a, sizeof(vm_wrong_type_err));
