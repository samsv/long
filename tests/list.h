...
static inline void sv_test_list_init_get(sv_testing_t* t)
{
   ...
   sv_test_run(t, !ll_get(l, 99).is_some);
   sv_test_run(t, sv_opt_unwrap(ll_head(l)).number == 1);
   ll_deinit(&l, &sv_gpa);
   ...
   sv_test_run(t, sv_opt_unwrap(ll_get(from_vec, 0)).number == 7);
   sv_test_run(t, sv_opt_unwrap(ll_get(from_vec, 1)).number == 8);
   ll_deinit(&from_vec, &sv_gpa);

   list_t empty = ll_empty();
   sv_test_run(t, ll_count(empty) == 0);
   sv_test_run(t, !ll_head(empty).is_some);
...
static inline void sv_test_list_persistence(sv_testing_t* t)
{
   ...
   sv_test_run(t, sv_opt_unwrap(ll_get(l2, 0)).number == 1);
   sv_test_run(t, sv_opt_unwrap(ll_get(l2, 1)).number == 2);
   sv_test_run(t, ll_count(l1) == 2);
   sv_test_run(t, sv_opt_unwrap(ll_get(l1, 0)).number == 2);

   ll_deinit(&l1, &sv_gpa);
   sv_test_run(t, sv_opt_unwrap(ll_get(l2, 2)).number == 3);

   list_t l3 = ll_prepend(l2, sv_test_list_num(0), &sv_gpa);
   sv_test_run(t, ll_count(l3) == 4);
   ll_deinit(&l2, &sv_gpa);
   sv_test_run(t, sv_opt_unwrap(ll_get(l3, 0)).number == 0);
   sv_test_run(t, sv_opt_unwrap(ll_get(l3, 3)).number == 3);
   ll_deinit(&l3, &sv_gpa);

   list_t base = ll_empty();
   list_t one = ll_prepend(base, sv_test_list_num(9), &sv_gpa);
   sv_test_run(t, ll_count(one) == 1);
   sv_test_run(t, sv_opt_unwrap(ll_head(one)).number == 9);
   ...
   sv_test_run(t, sv_opt_unwrap(ll_get(joined, 0)).number == 1);
   sv_test_run(t, sv_opt_unwrap(ll_get(joined, 1)).number == 2);
   sv_test_run(t, sv_opt_unwrap(ll_get(joined, 2)).number == 3);
...
static inline void sv_test_list_add(sv_testing_t* t)
{
   ...
   sv_test_run(t, sv_opt_unwrap(ll_get(both, 0)).number == 1);
   sv_test_run(t, sv_opt_unwrap(ll_get(both, 3)).number == 4);
   ...
   sv_test_run(t, sv_opt_unwrap(ll_get(right_only, 1)).number == 4);
...
static inline void sv_test_list_modify(sv_testing_t* t)
{
   ...
   sv_test_run(t, sv_opt_unwrap(ll_get(inserted, 0)).number == 1);
   sv_test_run(t, sv_opt_unwrap(ll_get(inserted, 1)).number == 9);
   sv_test_run(t, sv_opt_unwrap(ll_get(inserted, 2)).number == 2);
   sv_test_run(t, ll_count(l) == 3);
   sv_test_run(t, sv_opt_unwrap(ll_get(l, 1)).number == 2);
   ...
   sv_test_run(t, sv_opt_unwrap(ll_get(updated, 1)).number == 9);
   sv_test_run(t, sv_opt_unwrap(ll_get(l, 1)).number == 2);
   ll_deinit(&updated, &sv_gpa);

   list_t deleted = ll_delete_at(l, 1, &sv_gpa);
   sv_test_run(t, ll_count(deleted) == 2);
   sv_test_run(t, sv_opt_unwrap(ll_get(deleted, 0)).number == 1);
   sv_test_run(t, sv_opt_unwrap(ll_get(deleted, 1)).number == 3);
   sv_test_run(t, ll_count(l) == 3);
...
static inline void sv_test_list_head_tail(sv_testing_t* t)
{
   ...
   sv_test_run(t, sv_opt_unwrap(ll_head(rest)).number == 2);
   sv_test_run(t, ll_count(l) == 3);

   list_t rest2 = ll_tail(rest, &sv_gpa);
   sv_test_run(t, ll_count(rest2) == 1);
   sv_test_run(t, sv_opt_unwrap(ll_head(rest2)).number == 3);

   list_t none = ll_tail(rest2, &sv_gpa);
   sv_test_run(t, none.cell != NULL);
   sv_test_run(t, ll_count(none) == 0);
   sv_test_run(t, !ll_head(none).is_some);
...
static inline void sv_test_list_iter(sv_testing_t* t)
{
   value_t vals[] = { sv_test_list_num(1), sv_test_list_num(2), sv_test_list_num(3) };
   list_t l = ll_init(vals, 3, &sv_gpa);
   list_t multi = ll_insert(l, sv_test_list_num(9), 1, &sv_gpa);

   ll_iter_t it = ll_iter_init(l);
   ll_deinit(&l, &sv_gpa);
   double expected[] = { 1, 2, 3 };
   int64_t seen = 0;
   int ok = 1;
   for (sv_opt_t(value_t) v = ll_iter_next(&it); v.is_some; v = ll_iter_next(&it)) {
      ok = ok && seen < 3 && v.value.number == expected[seen];
      seen++;
   }
   ll_iter_deinit(&it, &sv_gpa);
   sv_test_run(t, ok);
   sv_test_run(t, seen == 3);

   ll_iter_t it2 = ll_iter_init(multi);
   double expected2[] = { 1, 9, 2, 3 };
   seen = 0;
   ok = 1;
   for (sv_opt_t(value_t) v = ll_iter_next(&it2); v.is_some; v = ll_iter_next(&it2)) {
      ok = ok && seen < 4 && v.value.number == expected2[seen];
      seen++;
   }
   sv_test_run(t, ok);
   sv_test_run(t, seen == 4);
   sv_test_run(t, !ll_iter_next(&it2).is_some);
   ll_iter_deinit(&it2, &sv_gpa);
   ll_deinit(&multi, &sv_gpa);

   list_t empty = ll_empty();
   ll_iter_t it3 = ll_iter_init(empty);
   sv_test_run(t, !ll_iter_next(&it3).is_some);
   ll_iter_deinit(&it3, &sv_gpa);
   ll_deinit(&empty, &sv_gpa);
}
...
static inline void sv_test_list_values(sv_testing_t* t)
{
   ...
   sv_test_run(t, sv_opt_unwrap(ll_get(l, 0)).obj.cell == v.obj.cell);
   sv_test_run(t, sv_str_comp(sv_opt_unwrap(ll_get(l, 0)).obj.cell->value.str, sv_str_init("shared")));

   list_t l2 = ll_prepend(l, sv_test_list_num(1), &sv_gpa);
   sv_test_run(t, v.obj.cell->count >= 2);

   list_t replaced = ll_update(l, sv_test_list_num(0), 0, &sv_gpa);
   sv_test_run(t, sv_opt_unwrap(ll_get(replaced, 0)).kind == VALUE_NUMBER);
