...
static inline void sv_test_rc_counting(sv_testing_t* t)
{
   ...
   sv_rc_t(int) rc = rc_init(int, 42, sv_test_rc_free_int, &sv_gpa);
   sv_test_run(t, rc.cell != NULL);
   sv_test_run(t, rc.cell->count == 1);
   sv_test_run(t, *rc_get(rc) == 42);

   sv_rc_t(int) rc2 = rc_borrow(rc);
   sv_test_run(t, rc2.cell == rc.cell);
   sv_test_run(t, rc.cell->count == 2);
   sv_test_run(t, *rc_get(rc2) == 42);
   ...
   sv_rc_t(int) stale = rc_borrow(rc);
   sv_test_run(t, stale.cell == NULL);
   sv_test_run(t, rc_get(stale) == NULL);
...
static inline void sv_test_rc_owned_value(sv_testing_t* t)
{
   ...
   sv_rc_t(sv_str_t) rc = rc_init(sv_str_t, owned, sv_str_deinit, &sv_gpa);
   sv_test_run(t, rc.cell != NULL);

   sv_rc_t(sv_str_t) rc2 = rc_borrow(rc);
   sv_test_run(t, sv_str_comp(*rc_get(rc2), sv_str_init("counted")));

   rc_deinit(&rc2, &sv_gpa);
   sv_test_run(t, sv_str_comp(*rc_get(rc), sv_str_init("counted")));
...
static inline void sv_test_rc_oom(sv_testing_t* t)
{
   ...
   sv_rc_t(int) rc = rc_init(int, 7, sv_test_rc_free_int, &sv_test_fail_alloc);
   sv_test_run(t, rc.cell == NULL);

   sv_rc_t(int) rc2 = rc_borrow(rc);
   sv_test_run(t, rc2.cell == NULL);
   sv_test_run(t, rc_get(rc) == NULL);
   sv_test_run(t, rc_get(rc2) == NULL);

   rc_deinit(&rc, &sv_test_fail_alloc);
   rc_deinit(&rc2, &sv_test_fail_alloc);
   sv_test_run(t, sv_test_rc_frees == 0);
}
